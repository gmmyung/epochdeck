...
from runloom._json_normalization import (
...
def agent(
    ...
    *,
    ...
    completed = 0
    with RunloomClient(selected_server, timeout=30.0) as client:
...
class _Heartbeat(threading.Thread):
    def __init__(self, *, server_url: str, trial_id: str, agent_id: str) -> None:
        super().__init__(name=f"runloom-sweep-{trial_id[:8]}", daemon=True)
    ...
    def run(self) -> None:
        ...
        with RunloomClient(self._server_url, timeout=5.0) as client:
...
def _agent_state_path(
    ...
    root = Path(
        state_dir
        or os.environ.get("RUNLOOM_SWEEP_STATE_DIR")
        or Path.home() / ".local" / "share" / "runloom" / "sweep-agents"
    ).expanduser()
    identity = hashlib.sha256(f"{server_url}\0{sweep_id}\0{agent_id}".encode()).hexdigest()
    return root / f"{identity}.json"


def _complete_pending_trial(client: RunloomClient, pending: Mapping[str, Any]) -> None:
