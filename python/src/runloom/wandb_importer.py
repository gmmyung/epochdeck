...
from runloom._wandb_state import (
    Checkpoint,
    ImportCancellation,
    ImportCancelled,
    WandbImportError,
...
from runloom.artifact import _validate_component, _validate_manifest_size
    ...
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(
            ...
            thread_name_prefix="runloom-wandb-media",
        )
...
def import_wandb_runs(
    ...
    *,
    ...
    with checkpoint_process_lock(checkpoint_path):
        ...
        executor = ThreadPoolExecutor(max_workers=workers, thread_name_prefix="runloom-wandb")
        interrupted = False
        try:
...
def _import_one_run(
    ...
    if stored_run_id is not None and stored_run_id != run_id:
        raise WandbImportError("checkpoint has the wrong deterministic Runloom run ID")
    if checkpoint_complete and stored_run_id is None:
        raise WandbImportError("completed checkpoint has no deterministic Runloom run ID")
    ...
    try:
        if phase == _PHASE_HISTORY:
            ...
            history: Iterable[Any] = _retrying_wandb_reads(
        ...
        if media_pipeline is not None:
            try:
                ...
                if callable(add_note):
                    add_note(
                        "Runloom also failed while draining W&B media workers: "
...
def _import_file_chunk(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="runloom-wandb-") as raw_root:
        ...
        request = {
            "id": artifact_id,
            "name": f"wandb-run-{run_id[:8]}-files-{chunk_start // _FILE_CHUNK_SIZE:04d}",
            "type": "wandb-run-files",
            "description": "One shard of files preserved by the Runloom W&B importer.",
            "metadata": {
...
def _source_changed_error(source_id: str, when: str) -> WandbImportError:
    return WandbImportError(
        f"W&B run {source_id!r} changed {when}; the deterministic Runloom target may "
