...
) -> Result<Json<epochdeck_protocol::SweepRecord>, HttpError> {
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (agent, loss, expected_stop) in [("agent-a", 0.1, false), ("agent-b", 1.0, true)] {
            ...
            let heartbeat: epochdeck_protocol::SweepTrialRecord = response_json(
                router
                    .clone()
                    .oneshot(json_request(
                        "POST",
            ...
            let completed: epochdeck_protocol::SweepTrialRecord = response_json(
                router
                    .clone()
                    .oneshot(json_request(
                        "POST",
        ...
        let trial_detail: epochdeck_protocol::SweepTrialRecord = response_json(
            router
                .clone()
                .oneshot(
        ...
        let sweeps: epochdeck_protocol::SweepListResponse = response_json(
            router
                .clone()
                .oneshot(
